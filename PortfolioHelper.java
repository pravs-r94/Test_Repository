...
{
    Customer portfolio;
    public static ArrayList<Customer> mockData()

    {
        //static NewPortfolio[] newPortfolios = new NewPortfolio[2];
        ArrayList<Customer> customers = new ArrayList<>();
        ArrayList<PurchaseStock> neethuStocks = new ArrayList<>();
        ArrayList<PurchaseStock> gouthuStocks = new ArrayList<>();

        neethuStocks.add(0, new PurchaseStock("Apple", 5, 12.53, new SimpleDateFormat("12/25/2020")));
        neethuStocks.add(1, new PurchaseStock("Tesla", 5, 11.3, new SimpleDateFormat("12/25/2020")));
        gouthuStocks.add(0,new PurchaseStock("Apple", 5, 12.53, new SimpleDateFormat("12/25/2020")));
        gouthuStocks.add(1, new PurchaseStock("Tesla", 5, 11.3, new SimpleDateFormat("12/25/2020")));
    ...
    public static Customer buyStocks(ArrayList<Stock> stocks, String cName, SimpleDateFormat date, Customer customer)
    {

        ArrayList<PurchaseStock> purchaseStocks = new ArrayList<>();
        Scanner cStockName = new Scanner(System.in);  // Create a Scanner object
        System.out.println("Enter Stock Name");
        // to do check if stock exists, else send exception
            ...
            if (stocks.get(i).getsFullName().equals(stockName))
            {
                ...
                if (stocks.get(i).getQty() > stockUnits)
                {
                    ...
                    else
                    {
                       ...
                       /* updatedStocks[0] = existingStocks[0];
                        updatedStocks[1] = existingStocks[1];
                       */
                    ...
                    //   portfolio[0] = new Portfolio(cName, stockUnits, stocks[i].getsAvgPrice(), stockName, date);
                }
            }
